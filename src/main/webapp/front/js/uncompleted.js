...
$(document).ready(function(){
		//搜索订单
		$.get("/logisticsrepair/repairOrder/queryRepairOrderList", 
...
function allocation(id){
	...
	$.get("/logisticsrepair/user/getRepairManList", function(data, status) {
...
$("#allocationSubmit").click(function (){
	...
	$.get("/logisticsrepair/repairAllocation/allotRepairOrder",{
		orderId:orderId,
		userId:repairManId
	}, function(data, status) {
		if(status=='success'){
			if(data.code==200){
				...
				alert("操作成功！");
				location.href="uncompleted.html";
			}else{
