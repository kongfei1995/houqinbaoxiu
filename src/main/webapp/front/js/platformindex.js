...
$("#save").click(function() {
	...
	$.get("/logisticsrepair/placard/addPlacard", {
		title : title,
		content : content,
		...
		if (status = 'success') {
			...
			if (resp.code == 200) {
				alert("操作成功！");
				location.href="platformedit.html";
			} else {
...
// 退出
$("#exit").click(function() {
	$.get("/logisticsrepair/user/logout");
...
$(".tagsinput-primary").on('itemRemoved', function(event) {
	...
	// 删除数据库tag
		...
		$.get("/logisticsrepair/repairCategory/removeRepairCategory", {
			name : tag
		}, function(data, status) {
			if (status = 'success') {
				// Success!
...
$(".tagsinput-primary").on('itemAdded', function(event) {
	...
	// 添加数据库tag
		$.get("/logisticsrepair/repairCategory/addRepairCategory", {
			name : tag
		}, function(data, status) {
			if (status = 'success') {
				// Success!
...
$(document).ready(function() {
	// 初始化分类tag
	$.get("/logisticsrepair/repairCategory/getRepairCategoryList", {
	...
	var categoryHtml = "";
	$.get("/logisticsrepair/index/getIndexInfo", function(data, status) {
		// 获取维修分类
		/*
