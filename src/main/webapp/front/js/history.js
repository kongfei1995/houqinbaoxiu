...
$(document).ready(function(){
		//搜索订单
		$.get("/logisticsrepair/repairOrder/getMyRepairOrderList", 
...
function complete(id){
	$.get("/logisticsrepair/repairOrder/completeOrder",{
		orderId:id
	}, function(data, status) {
		if(status=='success'){
			if(data.code==200){
...
function comment(orderId){
	...
	console.log(rat);
	$.get("/logisticsrepair/repairOrder/commentOrder",{
		orderId:orderId,
		star:rat,
		comment:""
	}, function(data, status) {
...
function deleteOrder(orderId){
	if(orderId!=""){
		$.get("/logisticsrepair/repairOrder/deleteOrder",{
			orderId:orderId
		}, function(data, status) {
			if(status=='success'){
				if(data.code==200){
...
function toHtml(order){
    ...
    if(order.status==1){
    	...
    	html+=' <div class="col-md-3"> <button class="btn btn-primary"  onclick=deleteOrder('+order.id+')>取消维修单</button> </div> ';
    }
    if(order.status==2){
    	//受理后，暂不允许修改
    	//html+=' <div class="col-md-3"> <button class="btn btn-primary"  onclick=updateOrder('+order.id+')>修改维修单</button> </div> ';
